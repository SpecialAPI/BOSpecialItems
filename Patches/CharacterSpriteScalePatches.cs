...
namespace BOSpecialItems.Patches
{
    ...
    public static class CharacterSpriteScalePatches
    {
        ...
        public static void InformationZoneReset(InformationZoneLayout __instance)
        {
            ...
            var img = __instance._unitPortrait.transform.Find($"{Plugin.GUID}_AdvancedUnitPortrait");
            if(img != null)
        ...
        public static void InformationZoneChanges(InformationZoneLayout __instance, CharacterCombatUIInfo character)
        {
            ...
            if(charso is CharacterSOAdvanced ch)
            {
                ...
                var imgTransform = __instance._unitPortrait.transform.Find($"{Plugin.GUID}_AdvancedUnitPortrait");
                GameObject img;
                ...
                if(imgTransform == null)
                {
                    ...
                    img.name = $"{Plugin.GUID}_AdvancedUnitPortrait";
                ...
                {
                    advimg.sprite = ch.combatMenuSprite;

                    imgTransform.localScale = ch.combatMenuSpriteScale;
                    imgTransform.localPosition = img.GetComponent<LocalPositionConstantSetter>().targetPos = ch.combatMenuSpriteOffset * (94f / 107.5f);
                }
                else
                {
                    advimg.sprite = character.Portrait;

                    imgTransform.localScale = ch.spriteScale;
                    imgTransform.localPosition = img.GetComponent<LocalPositionConstantSetter>().targetPos = ch.spriteOffset * (94f / 107.5f);
                }
        ...
        public static void ChangeImageSizeAndPosition(Image img, CharacterSO current, CharacterSO previous, float offsetMultiplier = 1f, bool menu = true, Transform overrideXOffsetTransform = null, bool doPrevious = true)
        {
            ...
            if(previous != null && previous is CharacterSOAdvanced prevCh && doPrevious)
            {
                ...
                img.transform.localScale = Vector3Divide(img.transform.localScale, useMenu ? prevCh.menuSpriteScale : prevCh.spriteScale);
